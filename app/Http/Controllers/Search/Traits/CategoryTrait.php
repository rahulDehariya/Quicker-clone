...
trait CategoryTrait
{
	...
	public function getCategory()
	{
		...
		if (Str::contains(Route::currentRouteAction(), 'Search\CategoryController')) {
			if (!config('settings.seo.multi_countries_urls')) {
				$parentCatSlug = null;
				$catSlug = getSegment(2);
				if (getSegment(3)) {
					$parentCatSlug = getSegment(2);
					$catSlug = getSegment(3);
				}
			} else {
				$parentCatSlug = null;
				$catSlug = getSegment(3);
				if (getSegment(4)) {
					$parentCatSlug = getSegment(3);
					$catSlug = getSegment(4);
				}
			}
	...
	public function getCategoryBySlug($catSlug, $parentCatSlug = null)
	{
		if (!empty($parentCatSlug)) {
			...
			$cat = Cache::remember($cacheId, $this->cacheExpiration, function () use ($catSlug) {
				$cat = Category::trans()->with(['parent', 'children'])->where('slug', '=', $catSlug)->first();
	...
	public function getRootCategories()
	{
		...
		$cats = Cache::remember($cacheId, $this->cacheExpiration, function () {
			$cats = Category::trans()->where('parent_id', 0)->get();
	...
	private function bindCatVariables($cat)
	{
		...
		// Get Custom Fields
		$customFields = CategoryField::getFields($this->cat->tid);
