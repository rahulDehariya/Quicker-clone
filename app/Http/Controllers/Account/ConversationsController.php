...
class ConversationsController extends AccountBaseController
{
	...
	public function index()
	{
		...
		MetaTag::set('description', t('Conversations Received on', ['appName' => config('settings.app.app_name')]));
		
		return appView('account.conversations', $data);
	}
	...
	public function messages($conversationId)
	{
		...
		MetaTag::set('description', t('Messages Received on', ['appName' => config('settings.app.app_name')]));
		
		return appView('account.messages', $data);
	}
	...
	public function reply($conversationId, ReplyMessageRequest $request)
	{
		...
		try {
			...
			flash(t("Your reply has been sent"))->success();
		} catch (\Exception $e) {
