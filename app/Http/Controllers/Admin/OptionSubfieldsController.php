...
class OptionSubfieldsController extends PanelController
{
	...
	public function setup()
	{
		...
		$field = FieldOption::findTransOrFail($this->fieldId);
		/*
		|--------------------------------------------------------------------------
		...
		*/
		...
		$this->xPanel->setRoute(admin_uri('custom_fields/' . $field->id . '/subfields'));
		$this->xPanel->setEntityNameStrings(
			trans('admin.subfiled') . ' &rarr; ' . '<strong>' . $field->value . '</strong>',
			trans('admin.subfields') . ' &rarr; ' . '<strong>' . $field->value . '</strong>'
		);
