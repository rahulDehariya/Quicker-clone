...
trait EditTrait
{
    ...
    /**
	 * Update uncompleted Post Through API
	 *
	 * @param $tmp_token
	 * @param PostRequest $request
	 * @return \Illuminate\Http\RedirectResponse|\Illuminate\Routing\Redirector
	 */
    public function UpdatePostForm($postIdOrToken="",  $request){

        
        $user_id=(Auth::guard('api')->user()->id)?Auth::guard('api')->user()->id:"";

        if(!empty($postIdOrToken)){
      // Get Post through token and post id
         $post = Post::currentCountry()->with(['city'])
				->withoutGlobalScopes([VerifiedScope::class, ReviewedScope::class])
                ->where('tmp_token', $postIdOrToken)
				->first();
        ...
        return response()->json(['status'=>'200','message'=>'Your ad has been updated','record'=>$post]);
    }
    ...
    public function UpdatePost($postIdOrToken="",  $request){
