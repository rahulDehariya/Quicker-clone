...
class CreateController extends FrontController
{
	...
	public function postForm(PostRequest $request, $tmpToken = null)
	{
		...
		// Locale Path
		$localePath = !currentLocaleShouldBeHiddenInUrl() ? config('app.locale') . '/' : '';
		
		// Get Next URL
		$nextStepUrl = $localePath . 'posts/create/' . $post->tmp_token . '/photos';
		...
		if ($emailVerificationRequired || $phoneVerificationRequired) {
			...
			if ($phoneVerificationRequired) {
				...
				$nextStepUrl = $localePath . 'verify/post/phone/';
			}
