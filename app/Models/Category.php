...
class Category extends BaseModel
{
	...
	*/
	...
	public function subCategoriesBtn($xPanel = false)
	{
		...
		$out .= mb_ucfirst(trans('admin.subcategories'));
		$out .= '</a>';
	...
	public function customFieldsBtn($xPanel = false)
	{
		...
		$out .= mb_ucfirst(trans('admin.custom fields'));
		$out .= '</a>';
	...
	public static function selectBoxTree($skippedId = null, $entries = [], &$tab = [], $level = 0, $spacerChars = '-----')
	{
		if (empty($entries)) {
			...
			$entries = self::trans()->where('parent_id', 0)->where('id', '!=', $skippedId)->orderBy('lft')->get();
			if ($entries->count() <= 0) {
				return [];
			}
		}
	...
	{
		$cat = self::findTrans($catId);
		if (!empty($cat)) {
			$parentIds[$cat->tid] = $cat->tid;
			if ($cat->parent_id != 0) {
	...
	{
		$sql = 'SELECT parent.translation_of, COUNT(*) AS total
				FROM ' . DBTool::table('categories') . ' AS node,
						' . DBTool::table('categories') . ' AS parent,
						' . DBTool::table('posts') . ' AS tPost
				WHERE node.lft BETWEEN parent.lft AND parent.rgt
						AND node.translation_of = tPost.category_id
						...
						AND parent.translation_lang = :langCode
						AND node.translation_lang = :langCode
				GROUP BY parent.translation_of';
		$bindings = [
	...
	*/
	...
	{
		return $this->hasMany(Category::class, 'parent_id', 'translation_of')
	...
	{
		return $this->belongsTo(Category::class, 'parent_id', 'translation_of')
			->where('translation_lang', config('app.locale'))
			->with('parent');
	}
