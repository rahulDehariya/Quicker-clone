...
$(document).ready(function () {
    ...
    $(document).on('change', '.make_forsubfields', function (e) {
		e.preventDefault(); /* prevents the submit or reload */
		fieldId = $(this).val();
		if (typeof catId === 'undefined') {
			fieldId = 0;
		}
		getSubfields(siteUrl, languageCode, fieldId);
	});

    $(document).on('change', '.brand_forsubfields', function (e) {
		e.preventDefault(); /* prevents the submit or reload */
		fieldId = $(this).val();
		if (typeof catId === 'undefined') {
			fieldId = 0;
...
function getSubfields(siteUrl, languageCode, catId) {
	/* Make ajax call */
	$.ajax({
		method: 'POST',
		url: siteUrl + '/ajax/fields_options/field_id',
		data: {
		...
		if(obj){
			$(document).find('.model_forsubfields').html(obj.customFields)
		}
	});
}
...
function getMakeSelect(){
    console.log('herhh5ee');
    $('.Make-zyz').append('<option value="1">One</option><option value="1">One</option><option value="1">One</option><option value="1">One</option>');
    $('.Make-zyz').select2();
}
function getCustomFieldsByCategory(siteUrl, languageCode, catId) {
    console.log(catId+':2333eee');
