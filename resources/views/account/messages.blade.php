...
@section('content')
						...
						<div class="table-responsive">
							<form name="listForm" method="POST" action="{{ $conDelAllUrl }}">
								...
								<table id="addManageTable" class="table table-striped table-bordered add-manage-table table demo" data-filter="#filter" data-filter-text-only="true">
									...
									<tr>
										<td colspan="3">
											<strong>{{ t('sender_name') }}:</strong> {{ $conversation->from_name ?? '--' }}<br>
											<strong>{{ t('sender_email') }}:</strong> {{ $conversation->from_email ?? '--' }}<br>
											<strong>{{ t('sender_phone') }}:</strong> {{ $conversation->from_phone ?? '--' }}<br>
											<hr>
	...
	@if (isset($conversation) && $conversation->count() > 0)
		@includeFirst([config('larapen.core.customizedViewPath') . 'account.inc.reply-message', 'account.inc.reply-message'])
	@endif
